...
// static const int64_t nTargetSpacing = 2.5 * 60; // Guldencoin: 150 seconds


// Delta, the Guldencoin Difficulty Re-adjustment
// unsigned int static GetNextWorkRequired_Delta (const CBlockIndex* pindexLast, const CBlockHeader *pblock, int algo)
unsigned int static GetNextWorkRequired_DELTA (const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    int64_t nRetargetTimespan = nTargetSpacing;         // algo: actual default block time

    unsigned int nProofOfWorkLimit = bnProofOfWorkLimit.GetCompact();   // algo specific
    ...
    // adjust Time based on day intervall
